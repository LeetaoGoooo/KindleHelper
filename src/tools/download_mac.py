...
class Download:
    def __init__(self):
        ...
        self.down_nums = 2
        self.max_retry = 10
        self.download_dir = os.path.join(root,'src', 'downloads')
        ...
        with closing(requests.get(url, stream=True)) as response:
            ...
            with open(f'{self.download_dir}/{file_name}', 'wb') as f:
                for data in response.iter_content(chunk_size=chunk_size):
                    ...
                    if not trigger:
                        print(
                            f'\r 文件下载进度:{int(now_progress)}% 文件下载速度:{int(speed)} kb/s')
                    else:
                        print(
                            f'\r 文件下载进度:{int(now_progress)}% 文件下载速度:{int(speed)} kb/s')
                        trigger.emit({"progress": int(now_progress), "speed": int(
                            speed), "id": hashlib.md5(url.encode('utf-8')).hexdigest()})
        print('end...')
        return True
...
if __name__ == '__main__':
    ...
    url = 'http://21297300.174.ctc.data.tv002.com/down/6eadcbf3f95170f8533feabef555aaae/高效能人士的七个习惯%28全文%29.pdf?cts=dx-f-D1A193A68A117F9f395&ctp=1A193A68A117&ctt=1576520651&limit=1&spd=44000&ctk=6eadcbf3f95170f8533feabef555aaae&chk=e93f351bdbca2eaca3d2059bee49ad26-2869458&mtd=1'
